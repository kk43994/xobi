...
### 2.2 创建向导（/factory/detail）
- 复用：`xobixiangqing/frontend/src/pages/Home.tsx` 的创建逻辑（兼容旧入口：`/projects/new` → `/factory/detail`）
- 关键输入：
...
### 4.2 详情图工厂（/factory/detail）
定位：把“主图/素材”扩展为“详情页图集”，并复用 A 的 Project 工作流（大纲/文案/图片/导出）。

v1（可落地）：
- 输入：从 Excel 数据集行（DatasetItem）选择主图/素材图（优先 new_images）
- 动作：创建/打开对应 Project，并自动挂 Material（作为生成参考图）
- 产出：在项目工作台里一键生成（outline/desc/images）并导出 ZIP

说明（已按你最新口径调整）：
- `/factory/detail`：作为“详情图工厂”入口，直接复用朋友 `xobicc` 的详情页生成 UI（兼容旧入口：`/projects/new` → `/factory/detail`）
- `/factory/detail-bridge`：保留为“桥接页”，用于从 Excel/批量选中主图后，一键创建/打开 Project 再进入详情页生成流程

### 4.3 批量替换（/factory/batch）
复用来源：
- B 的 `/api/replace/batch/*`
- 前端可先复用静态 `batch-antdx.html` 的交互，再 React 化

### 4.4 风格批量（/factory/style-batch）
复用来源：
- B 的 `/api/style/batch/*`
