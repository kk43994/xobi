...
# Project-scoped cache (keyed by effective config signature)
_project_ai_service_cache: dict = {}
...
def get_ai_service_for_project(project_id: str) -> AIService:
    """
    Get an AIService instance using effective per-project settings.

    Notes:
    - This does NOT mutate Flask app.config (thread-safe).
    - Provider instances are cached by a config signature (format/base/key-hash/model).
    """
    ...
    if not pid:
        # Fallback to global singleton when project is not specified.
        return get_ai_service()

    global_settings = Settings.get_settings()
    project_settings = ProjectSettings.query.get(pid)

    provider_format = (
        (getattr(project_settings, "ai_provider_format", None) or "").strip().lower()
        or (getattr(global_settings, "ai_provider_format", None) or "").strip().lower()
        or "gemini"
    )

    api_base = (getattr(project_settings, "api_base_url", None) if project_settings else None) or global_settings.api_base_url
    if api_base is not None and str(api_base).strip() == "":
        api_base = None

    api_key = (getattr(project_settings, "api_key", None) if project_settings else None) or global_settings.api_key
    if api_key is not None and str(api_key).strip() == "":
        api_key = None

    cfg = get_config()
    if has_app_context() and current_app and hasattr(current_app, "config"):
        default_text_model = current_app.config.get("TEXT_MODEL", cfg.TEXT_MODEL)
        default_image_model = current_app.config.get("IMAGE_MODEL", cfg.IMAGE_MODEL)
    else:
        default_text_model = cfg.TEXT_MODEL
        default_image_model = cfg.IMAGE_MODEL

    text_model = (getattr(project_settings, "text_model", None) if project_settings else None) or global_settings.text_model
    image_model = (getattr(project_settings, "image_model", None) if project_settings else None) or global_settings.image_model
    text_model = str(text_model).strip() if text_model else str(default_text_model)
    image_model = str(image_model).strip() if image_model else str(default_image_model)

    if provider_format == "openai" and api_base:
        api_base = normalize_openai_api_base(str(api_base))

    key_hash = sha256(str(api_key or "").encode("utf-8")).hexdigest()[:12] if api_key else "nokey"
    cache_key = "|".join(
        [
            "project",
            pid,
    ...
    with _cache_lock:
        cached = _project_ai_service_cache.get(cache_key)
        if cached is not None:
            return cached

        if provider_format == "openai":
            text_provider = OpenAITextProvider(api_key=str(api_key or ""), api_base=str(api_base or "") or None, model=text_model)
            image_provider = OpenAIImageProvider(api_key=str(api_key or ""), api_base=str(api_base or "") or None, model=image_model)
        else:
            text_provider = GenAITextProvider(api_key=str(api_key or ""), api_base=str(api_base or "") or None, model=text_model)
            image_provider = GenAIImageProvider(api_key=str(api_key or ""), api_base=str(api_base or "") or None, model=image_model)

        svc = AIService(text_provider=text_provider, image_provider=image_provider)
        _project_ai_service_cache[cache_key] = svc
        return svc
...
def clear_ai_service_cache():
    ...
    with _lock:
        ...
        with _cache_lock:
            ...
            _project_ai_service_cache.clear()
