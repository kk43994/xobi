...
from .ecom_prompts import (
    get_ecom_outline_generation_prompt,
    get_ecom_page_description_prompt,
    get_ecom_image_generation_prompt,
)
...
def _detect_non_electronic_hint(text: str) -> bool:
    raw = (text or "").strip()
    if not raw:
        return False
    if re.search(r"电子部件\s*=\s*无", raw, flags=re.IGNORECASE):
        return True
    return bool(re.search(r"(毛绒|布偶|布娃娃|玩偶|公仔)", raw)) and not bool(
        re.search(r"电子部件\s*=\s*有", raw, flags=re.IGNORECASE)
    )


def _detect_non_electronic_hint_from_context(project_context: "ProjectContext", extra_text: str = "") -> bool:
    parts: List[str] = []
    for value in (
        getattr(project_context, "idea_prompt", None),
        getattr(project_context, "outline_text", None),
        getattr(project_context, "description_text", None),
        extra_text,
    ):
        if value:
            parts.append(str(value))
    return _detect_non_electronic_hint("\n".join(parts))


def _sanitize_text_snippet(s: str) -> str:
    if not s:
        return s
    out = str(s)
    for term in _PPT_LIKE_TERMS:
        out = re.sub(re.escape(term), "", out, flags=re.IGNORECASE)
    return out.strip()


def _contains_any_term(s: str, terms: List[str]) -> bool:
    if not s:
        return False
    upper = str(s).upper()
    for term in terms:
        if str(term).upper() in upper:
            return True
    return False


def _sanitize_outline(outline: List[Dict], non_electronic_hint: bool) -> List[Dict]:
    def _sanitize_page(page: Dict) -> Dict:
        page = dict(page or {})
        page["title"] = _sanitize_text_snippet(page.get("title", ""))
        points = page.get("points") or []
        cleaned_points = []
        if isinstance(points, list):
            for p in points:
                p_str = _sanitize_text_snippet(str(p))
                if not p_str:
                    continue
                if non_electronic_hint and _contains_any_term(p_str, _ELECTRONIC_TERMS):
                    continue
                cleaned_points.append(p_str)
        page["points"] = cleaned_points
        return page
    ...
    for item in outline or []:
        if not isinstance(item, dict):
            continue
        if "part" in item and "pages" in item and isinstance(item.get("pages"), list):
            part = _sanitize_text_snippet(str(item.get("part", "")))
            pages = [_sanitize_page(p) for p in item.get("pages") or []]
            cleaned.append({"part": part, "pages": pages})
        else:
            cleaned.append(_sanitize_page(item))
    return cleaned


def _sanitize_page_description(text: str, non_electronic_hint: bool) -> str:
    if not text:
        return text
    lines = []
    for line in str(text).splitlines():
        ...
        if not raw:
            lines.append(line)
            continue
        if _contains_any_term(raw, _PPT_LIKE_TERMS):
            continue
        if non_electronic_hint and _contains_any_term(raw, _ELECTRONIC_TERMS):
            continue
        # Inline sanitization (best-effort)
        cleaned_line = _sanitize_text_snippet(line)
        lines.append(cleaned_line if cleaned_line else "")
    return "\n".join(lines).strip()
...
class ProjectContext:
    ...
    def __init__(self, project_or_dict, reference_files_content: Optional[List[Dict[str, str]]] = None):
        ...
        if hasattr(project_or_dict, 'idea_prompt'):
            # 是 Project 对象
            self.idea_prompt = sanitize_prompt_text(project_or_dict.idea_prompt)
            ...
            self.project_type = 'ecom'
            self.page_aspect_ratio = getattr(project_or_dict, 'page_aspect_ratio', None)
            self.cover_aspect_ratio = getattr(project_or_dict, 'cover_aspect_ratio', None)
        else:
            # 是字典
            self.idea_prompt = sanitize_prompt_text(project_or_dict.get('idea_prompt'))
            ...
            self.project_type = 'ecom'
            self.page_aspect_ratio = project_or_dict.get('page_aspect_ratio')
            self.cover_aspect_ratio = project_or_dict.get('cover_aspect_ratio')
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            'page_aspect_ratio': self.page_aspect_ratio,
            'cover_aspect_ratio': self.cover_aspect_ratio,
...
class AIService:
    ...
    def generate_outline(self, project_context: ProjectContext, language: str = None) -> List[Dict]:
        """
        Generate image set outline from idea prompt
        ...
        """
        outline_prompt = get_ecom_outline_generation_prompt(project_context, language)
        outline = self.generate_json(outline_prompt, thinking_budget=1000)

        non_electronic_hint = _detect_non_electronic_hint_from_context(project_context)
        if isinstance(outline, list):
            outline = _sanitize_outline(outline, non_electronic_hint)
        return outline
    ...
    def generate_page_description(self, project_context: ProjectContext, outline: List[Dict], 
        ...
        desc_prompt = get_ecom_page_description_prompt(
        ...
        non_electronic_hint = _detect_non_electronic_hint_from_context(project_context)
        cleaned = _sanitize_page_description(response_text, non_electronic_hint)
        return dedent(cleaned or "")
    ...
    def generate_image_prompt(self, outline: List[Dict], page: Dict, 
        ...
        non_electronic_hint = _detect_non_electronic_hint((extra_requirements or "") + "\n" + (cleaned_page_desc or ""))
        cleaned_page_desc = _sanitize_page_description(cleaned_page_desc, non_electronic_hint) or ""
        
        prompt = get_ecom_image_generation_prompt(
    ...
    def parse_description_to_page_descriptions(self, project_context: ProjectContext, 
        ...
        if isinstance(descriptions, list):
            ...
            for desc in descriptions:
                text = str(desc)
                cleaned.append(_sanitize_page_description(text, non_electronic_hint) or "")
            return cleaned
        else:
    ...
    def refine_outline(self, current_outline: List[Dict], user_requirement: str,
        ...
        non_electronic_hint = _detect_non_electronic_hint_from_context(project_context, user_requirement or "")
        if isinstance(outline, list):
            outline = _sanitize_outline(outline, non_electronic_hint)
        return outline
