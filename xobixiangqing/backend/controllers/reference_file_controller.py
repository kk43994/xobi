...
from models import db, ReferenceFile, Project, ProjectSettings, Settings
...
def _parse_file_async(file_id: str, file_path: str, filename: str, app):
    ...
    with app.app_context():
        try:
            ...
            # Per-project overrides: if the file is associated with a project, use that project's API settings.
            global_settings = Settings.get_settings()
            project_settings = ProjectSettings.query.get(reference_file.project_id) if reference_file.project_id else None

            provider_format = (
                (getattr(project_settings, "ai_provider_format", None) if project_settings else None)
                or global_settings.ai_provider_format
                or current_app.config.get("AI_PROVIDER_FORMAT", "gemini")
            )
            api_key = (
                (getattr(project_settings, "api_key", None) if project_settings else None)
                or global_settings.api_key
                or current_app.config.get("GOOGLE_API_KEY", "")
            )
            api_base = (
                (getattr(project_settings, "api_base_url", None) if project_settings else None)
                or global_settings.api_base_url
                or current_app.config.get("GOOGLE_API_BASE", "")
            )
            mineru_token = (
                (getattr(project_settings, "mineru_token", None) if project_settings else None)
                or global_settings.mineru_token
                or current_app.config.get("MINERU_TOKEN", "")
            )
            mineru_api_base = (
                (getattr(project_settings, "mineru_api_base", None) if project_settings else None)
                or global_settings.mineru_api_base
                or current_app.config.get("MINERU_API_BASE", "")
            )

            parser = FileParserService(
                mineru_token=mineru_token,
                mineru_api_base=mineru_api_base,
                # 当前架构下，统一 API Key/Base 同时注入到 google/openai 两条参数（FileParserService 内部按 provider_format 选择）
                google_api_key=api_key or "",
                google_api_base=api_base or "",
                openai_api_key=api_key or "",
                openai_api_base=api_base or "",
                image_caption_model=current_app.config['IMAGE_CAPTION_MODEL'],
                provider_format=str(provider_format or "gemini").lower(),
            )
