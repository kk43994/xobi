...
def update_page(project_id, page_id):
    """
    PUT /api/projects/{project_id}/pages/{page_id} - Update page fields (metadata).

    Supported fields:
    - part: string | null
    - aspect_ratio: "w:h" | "" | null (empty/null clears per-page override)
    """
    try:
        ...
        if 'part' in data:
            ...
            part = str(raw_part).strip() if raw_part is not None else ''
            page.part = part or None
