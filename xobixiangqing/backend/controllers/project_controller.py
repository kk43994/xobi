...
from models import db, Project, Page, Task, ReferenceFile, Material
from utils import success_response, error_response, not_found, bad_request
from utils.text_sanitize import sanitize_prompt_text
...
def _check_project_access(project, current_user):
    """
    ...
    - User can access projects with user_id=None (legacy/shared projects)
    """
...
def _extract_image_urls_from_markdown(text: str) -> list[str]:
    """
    Best-effort: extract markdown image urls: ![alt](url)

    Mirrors AIService.extract_image_urls_from_markdown without instantiating providers.
    """
    import re
    ...
    for url in matches:
        ...
        if not u:
            continue
        if u.startswith('http://') or u.startswith('https://') or u.startswith('/files/mineru/'):
            urls.append(u)
    return urls


@project_bp.route('/<project_id>/debug/generation-plan', methods=['GET'])
def debug_generation_plan(project_id):
    """
    GET /api/projects/{project_id}/debug/generation-plan

    A no-AI, no-token endpoint that returns which reference images / aspect ratios
    the backend will use when generating images.
    ...
    try:
        ...
        raw_use_template = (request.args.get('use_template') or 'true').strip().lower()
        use_template = raw_use_template in ('1', 'true', 'yes', 'y', 'on')
        ...
        material_urls = [m.url for m in materials if getattr(m, 'url', None)]
        product_primary_url = material_urls[0] if material_urls else None
        ...
        for page in pages:
            ...
            ratio_override = (getattr(page, "aspect_ratio", None) or "").strip() or None
            effective_ratio = ratio_override or (
                project.cover_aspect_ratio if page.order_index == 0 else project.page_aspect_ratio
            )

            desc_content = page.get_description_content() or {}
            desc_text = desc_content.get('text', '') if isinstance(desc_content, dict) else ''
            if not desc_text and isinstance(desc_content, dict) and desc_content.get('text_content'):
            ...
            desc_image_urls = _extract_image_urls_from_markdown(desc_text)

            # Predict ref strategy (matches task_manager behavior):
            # - If product exists: primary=product, secondary=board/template if template exists.
            # - Else: primary=template (if exists), otherwise only desc images.
            if product_primary_url:
                predicted_primary = 'product'
                predicted_secondary = 'board_or_template' if template_exists else 'none'
            else:
                predicted_primary = 'template' if template_exists else 'none'
                predicted_secondary = 'desc_images' if desc_image_urls else 'none'
            ...
            pages_info.append(
                {
                    ...
                    "title": title,
                    "aspect_ratio_override": ratio_override,
                    "effective_aspect_ratio": effective_ratio,
                    "has_description": bool(desc_text.strip()),
                    "desc_image_urls_count": len(desc_image_urls),
                    "predicted_primary_ref": predicted_primary,
                    "predicted_secondary_ref": predicted_secondary,
                }
            )
        ...
        return success_response(
            data={
                ...
                "page_aspect_ratio": project.page_aspect_ratio,
                "cover_aspect_ratio": project.cover_aspect_ratio,
                "default_resolution": current_app.config.get('DEFAULT_RESOLUTION'),
                "default_aspect_ratio": current_app.config.get('DEFAULT_ASPECT_RATIO'),
                "use_template": use_template,
                "template_exists": template_exists,
                "has_template_style": bool((project.template_style or "").strip()),
                "materials_count": len(material_urls),
                "product_primary_url": product_primary_url,
                "material_urls_sample": material_urls[:6],
                "pages": pages_info,
            },
            message="Generation plan (no AI calls)",
        )

    except Exception as e:
        logger.error("debug_generation_plan failed: %s", e, exc_info=True)
...
def create_project():
    ...
    Request body:
    {
        ...
        "project_type": "ecom",
        "page_aspect_ratio": "3:4|1:1|...",
        "cover_aspect_ratio": "1:1|3:4|..."
    }
    """
    try:
        current_user = get_current_user()
        ...
        # project_type is optional (default: ecom)
        project_type = (data.get('project_type') or 'ecom').strip().lower()
        if project_type != 'ecom':
            project_type = 'ecom'

        # Aspect ratio settings (project-level)
        page_aspect_ratio = (data.get('page_aspect_ratio') or '3:4').strip()
        cover_aspect_ratio = (data.get('cover_aspect_ratio') or '1:1').strip()
        ...
        if not _is_valid_ratio(page_aspect_ratio):
            return bad_request("Invalid page_aspect_ratio")
        if not _is_valid_ratio(cover_aspect_ratio):
            return bad_request("Invalid cover_aspect_ratio")
        ...
        project = Project(
            ...
            idea_prompt=sanitize_prompt_text(data.get('idea_prompt')),
            outline_text=sanitize_prompt_text(data.get('outline_text')),
            description_text=sanitize_prompt_text(data.get('description_text')),
...
def get_project(project_id):
    ...
    try:
        ...
        if not _check_project_access(project, current_user):
            return error_response('FORBIDDEN', '无权访问此项目', 403)
...
def update_project(project_id):
    ...
    try:
        ...
        if 'idea_prompt' in data:
            project.idea_prompt = sanitize_prompt_text(data['idea_prompt'])
        ...
        # Project type is locked to e-commerce images
        if 'project_type' in data and data['project_type']:
            project.project_type = 'ecom'
        ...
        if 'page_aspect_ratio' in data and data['page_aspect_ratio']:
            page_aspect_ratio = str(data['page_aspect_ratio']).strip()
            if not _is_valid_ratio(page_aspect_ratio):
                return bad_request("Invalid page_aspect_ratio")
            project.page_aspect_ratio = page_aspect_ratio

        if 'cover_aspect_ratio' in data and data['cover_aspect_ratio']:
            cover_aspect_ratio = str(data['cover_aspect_ratio']).strip()
            if not _is_valid_ratio(cover_aspect_ratio):
                return bad_request("Invalid cover_aspect_ratio")
            project.cover_aspect_ratio = cover_aspect_ratio
...
def delete_project(project_id):
    ...
    try:
        ...
        # 检查访问权限
        if not _check_project_access(project, current_user):
            return error_response('FORBIDDEN', '无权删除此项目', 403)
...
def generate_outline(project_id):
    ...
    try:
        ...
        if project.creation_type == 'outline':
            ...
            # 一句话生成：从idea生成大纲
            idea_prompt = sanitize_prompt_text(data.get('idea_prompt') or project.idea_prompt)
            ...
            logger.info("【大纲生成】使用 idea_prompt (len=%d)", len(project.idea_prompt or ""))
            
            reference_files_content = _get_project_reference_files_content(project_id)
            if reference_files_content:
                print(f"【大纲生成】发现 {len(reference_files_content)} 个参考文件:")
                for rf in reference_files_content:
                    print(f"   - {rf['filename']} ({len(rf['content'])}  chars)")
            else:
                print("【大纲生成】无参考文件")
        ...
        # 输出大纲生成结果到控制台
        print("\n" + "="*60)
        print(f"【大纲生成完成】项目 {project_id}")
        print(f"   共生成 {len(pages_list)} 页")
        print("="*60)
        for i, page in enumerate(pages_list):
            outline_content = page.get_outline_content() or {}
            title = outline_content.get('title', '（无标题）')
            points = outline_content.get('points', [])
            print(f"\n第 {i+1} 页: {title}")
            for j, point in enumerate(points[:3]):
                print(f"   · {point}")
            if len(points) > 3:
                print(f"   ... 还有 {len(points)-3} 个要点")
        print("\n" + "="*60 + "\n")
