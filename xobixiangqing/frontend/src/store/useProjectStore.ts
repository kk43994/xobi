...
interface ProjectState {
  ...
  initializeProject: (
    ...
    projectOptions?: { project_type?: 'ecom'; page_aspect_ratio?: string; cover_aspect_ratio?: string }
  ) => Promise<void>;
...
const debouncedUpdatePage = debounce(
  ...
  // 初始化项目
  initializeProject: async (type, content, templateImage, templateStyle, projectOptions) => {
    ...
    try {
      ...
      // 透传项目级选项（项目类型/比例等）
      if (projectOptions) {
        Object.assign(request, projectOptions);
      }
  ...
  generatePageDescription: async (pageId: string) => {
    ...
    try {
      ...
      await api.generatePageDescription(projectId, pageId, true);
  ...
  generateImages: async () => {
    ...
    await startAsyncTask(() => api.generateImages(projectId));
  },
  ...
  exportImagesZip: async () => {
    ...
    set({ isGlobalLoading: true, error: null });
    try {
      const response = await api.exportImagesZip(projectId);
      const downloadUrl =
